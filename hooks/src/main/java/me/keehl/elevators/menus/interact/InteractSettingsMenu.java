...
public class InteractSettingsMenu {
    ...
    public static void openInteractSettingsMenu(Player player, Elevator elevator) {
        ...
        List<ElevatorSetting<?>> settings = ElevatorSettingService.getElevatorSettings().stream().filter(i -> !i.isSettingGlobalOnly(elevator)).toList();

        int itemAmount = settings.size();
        List<ElevatorAction> upActions = ElevatorMenuHelper.getActionsWithSettings(elevator, true);
        List<ElevatorAction> downActions = ElevatorMenuHelper.getActionsWithSettings(elevator, false);
