...
public abstract class UniDialogHook extends DialogHook implements Listener {
    ...
    public UniDialogHook() {

        ElevatorListenerService.registerEventExecutor(PlayerQuitEvent.class, EventPriority.NORMAL, (event) -> {

            if(!this.userDialogData.containsKey(event.getPlayer().getUniqueId()))
                return;

            this.userDialogData.get(event.getPlayer().getUniqueId()).onQuit.run();
            this.userDialogData.remove(event.getPlayer().getUniqueId());
        });
    ...
    void onConfirm(DialogPayload payload) {
        ...
        try {
            ...
            valid = false;
            Elevators.log(Level.SEVERE, "Failed to validate dialog input. Please create an issue ticket on my GitHub if one doesn't already exist: https://github.com/keehl254/Elevators/issues. Issue:\n" + ResourceHelper.cleanTrace(ex));
        }
        ...
        try {
            ...
            Elevators.log(Level.SEVERE, "Failed to execute dialog input consumer. Please create an issue ticket on my GitHub if one doesn't already exist: https://github.com/keehl254/Elevators/issues. Issue:\n" + ResourceHelper.cleanTrace(ex));
        }
    ...
    public record ElevatorDialogData(Function<String, Boolean> validationFunction, Consumer<String> resultConsumer, Runnable onCancel, Runnable onQuit, DialogOpener dialogOpener) {
    }
