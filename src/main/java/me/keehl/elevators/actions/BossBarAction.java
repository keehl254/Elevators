...
public class BossBarAction extends ElevatorAction {
    ...
    public BossBarAction(JavaPlugin plugin, IElevatorType elevatorType, String key) {
    ...
    protected void onInitialize(String value) {
        ...
        IElevatorActionSetting<String> messageSetting = this.mapSetting(messageGrouping, "message","Message", desc, Material.WRITABLE_BOOK, ChatColor.GOLD, true);
        messageSetting.onClick(this::editMessage);

        desc = "This option controls the segments of the boss bar.";
        IElevatorActionSetting<BarStyle> styleSetting = this.mapSetting(barStyleGrouping, "style","Segments", desc, Material.SHULKER_SHELL, ChatColor.LIGHT_PURPLE, true);
        ...
        IElevatorActionSetting<BarColor> colorSetting = this.mapSetting(barColorGrouping, "color","Color", desc, Material.LIGHT_BLUE_DYE, ChatColor.AQUA, true);
    ...
    public static BossBar getPlayerBar(Player player, BossBarAction action, IElevator elevator) {
    ...
    public Runnable displayMessage(Player player, IElevator elevator, Supplier<String> message, double progress) {
        if (((Double) progress).isNaN())
            progress = 0.0;
        else if (((Double) progress).isInfinite())
            progress = 1.0;
