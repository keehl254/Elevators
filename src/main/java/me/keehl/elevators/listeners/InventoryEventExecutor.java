...
public class InventoryEventExecutor {
    ...
    public static void updateStackOnClick(InventoryClickEvent event) {
        ...
        if(clickedItem == null)
            return;
        IElevatorType elevatorType = ElevatorHelper.getElevatorType(clickedItem);
    ...
    public static void onAnvilPrepare(PrepareAnvilEvent e) {
        ...
        if (item == null)
            return;
        IElevatorType type = ElevatorHelper.getElevatorType(item);
        if (type == null)
            return;
        ...
        Elevators.getDataContainerService().dumpDataFromItemIntoItem(item, newElevator);
    ...
    public static void onCraft(CraftItemEvent e) {
        ...
        if (isElevatorCraftingRecipe) {
            ...
            if (!ElevatorPermHelper.canCraftElevatorType(elevatorType, player, (Recipe & Keyed) e.getRecipe())) {
                Elevators.getLocale().getCantCreateMessage().sendFormatted(player, new ElevatorEventData(player, elevatorType));
                e.setCancelled(true);
            }
        } else if (!ElevatorPermHelper.canDyeElevatorType(elevatorType, player, dyeColor)) {
            Elevators.getLocale().getCantDyeMessage().sendFormatted(player, new ElevatorEventData(player, elevatorType));
            e.setCancelled(true);
        }
    }
