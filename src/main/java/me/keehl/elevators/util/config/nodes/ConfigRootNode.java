...
public class ConfigRootNode<T extends Config> implements IConfigRootNode<T> {
    ...
    public Object serializeToObject() {
        ...
        if (converter != null) {
            try {
                return converter.serializeValueToYamlObject(this.config);
            } catch (Exception ignored) {
            }
        }

        ElevatorsAPI.log(Level.WARNING, "Failed to convert main root node! Using old data.");
        ...
        for (ConfigNode<?> childNode : this.getChildren()) {
            try {
                ...
                ElevatorsAPI.log(Level.SEVERE, "Failed to save config path \"" + childNode.getPath() + ".\". Please create an issue ticket on my GitHub if one doesn't already exist: https://github.com/keehl254/Elevators/issues. Issue:\n" + ResourceHelper.cleanTrace(e));
            }
        }
