...
public class InteractProtectMenu {
    ...
    public static void openInteractProtectMenu(Player player, IElevator elevator) {
        ...
        List<IProtectionHook> protectionHooks = Elevators.getHooksService().getProtectionHooks().stream().filter(i -> i.getConfig().doesAllowCustomization()).filter(i -> i.createIconForElevator(player, elevator) != null).collect(Collectors.toList());
        ...
        for (Map.Entry<IProtectionHook, Integer> hookData : ElevatorMenuHelper.mapToInventorySlot(protectionHooks).entrySet()) {
