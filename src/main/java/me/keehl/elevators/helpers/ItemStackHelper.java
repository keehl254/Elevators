...
public class ItemStackHelper {
    ...
    public static boolean isNotShulkerBox(Material type) {
        return !TagHelper.SHULKER_BOXES.isTagged(type);
    }
    ...
    public static ItemStack createItemStackFromElevatorType(IElevatorType elevatorType, DyeColor dyeColor) {
    ...
    public static void updateElevator(IElevatorType elevatorType, ItemStack currentStack) {
    ...
    public static void giveElevator(Item itemEntity, Inventory inv) {
        ...
        if (elevatorType == null)
            return;
        Elevators.getDataContainerService().updateItemStackFromV2(item, elevatorType);
    ...
    public static Map<ItemStack, Integer> addElevatorToInventory(IElevatorType elevatorType, int itemAmount, Material dyeMaterial, Inventory inventory) {
        List<String> lore = elevatorType.getLore().stream().map(ILocaleComponent::toLegacyText).toList();
        String displayName = elevatorType.getDisplayName().toLegacyText();
        return ItemStackHelper.addElevatorToInventory(elevatorType, itemAmount, dyeMaterial, inventory, displayName, lore);
    }

    public static Map<ItemStack, Integer> addElevatorToInventory(IElevatorType elevatorType, int itemAmount, Material dyeMaterial, Inventory inventory, String displayName, List<String> lore) {
