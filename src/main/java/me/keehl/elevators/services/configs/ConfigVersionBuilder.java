...
public class ConfigVersionBuilder {
    ...
    public static ConfigRootNode<ConfigRoot> getConfig(File configFile) {
        ...
        try {
            ...
            Map<?,?> data = (Map<?, ?>) converter.serializeValueToYamlObject(root);
            ...
            if(!backupFile.delete())
                ElevatorsAPI.log(Level.WARNING, "Failed to delete backup config file.");

            return rootNode;

        } catch (Exception e) {
            ElevatorsAPI.log(Level.SEVERE, "Error loading config. Using defaults. Please create an issue ticket on my GitHub with your config if you would like assistance: https://github.com/keehl254/Elevators/issues. Issue:\n" + ResourceHelper.cleanTrace(e));
        }
        ...
        if (backupFile != null) {
            try {
                ...
                ElevatorsAPI.log(Level.SEVERE, "Error reverting to old config. Config backup is available in the Elevators config path.");
            }
        }

        Elevators.getConfigService().invalidateConfig();
        try {
